...
<div class="sou-wrap">
    ...
    <div>
        <form action="/Beg/getList" method="GET">
...
<div id="beg-wrap">
    <?php if(isset($list)): ?><ul id="news">
                <?php if(is_array($list)): foreach($list as $key=>$vo): ?><li>
                        <h4 class="style"><?php echo ($vo["beg_title"]); ?></h4 >
                        <span class="beg-status">状态：<?php if(($vo["is_full"]) == "1"): ?>已完成<?php else: ?>未完成<?php endif; ?></span><span class="meny">￥<?php echo ($vo["price"]); ?></span>
                        <div class="clear"></div>
                        <p class="con"><?php echo ($vo["beg_desc"]); ?></p>
                        ...
                        <p class="locations_time"><span class="left"><a clstag="click|keycount|orderlist|ziyingchatim" title="联系他/她" href="#none" class="btn-im btn-im-jd left" onclick="addLetter(<?php echo ($vo["user_id"]); ?>, '<?php echo ($vo["user_name"]); ?>', <?php echo ((isset($_SESSION['shop']['user']) && ($_SESSION['shop']['user'] !== ""))?($_SESSION['shop']['user']):-1); ?>);"></a><a href="/User/showUser/user_id/<?php echo ($vo["user_id"]); ?>"><?php echo ($vo["user_name"]); ?></a></span>发布时间：<?php echo (date("Y-m-d",$vo["add_time"])); ?></p>
                        <p class="locations_beg">交易地点：<?php echo ($vo["address"]); ?></p>
                        <div class="clear"></div>
...
<footer>
      <a href="/">首页</a>
      <a href="/Beg/getList/beg_id/4?wap=0">电脑版</a>
