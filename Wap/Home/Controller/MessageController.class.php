...
class MessageController extends Controller
{
    ...
    public function showMessage ()
    {
        ...
        $page = new \Org\Util\Page(C('WAP_MESSAGE_PAGE_NUM'), false);

        $list = $message->where(['user_id' => session('user.user_id'), 'type' => $type])->limit($page->limit)->order('is_read asc,add_time desc')->select();
        ...
        $this->notReadLetterInfo = R('Letter/getNotReadLetter');
        ...
        $this->notReadCount = $this->notReadLetterInfo + $this->notReadOrderInfo + $this->notReadSystemInfo + $this->notReadSwitchInfo + $this->notReadCommentInfo;
    ...
    {
        return D('Message')->where(['user_id' => session('user.user_id'), 'type' => 3, 'is_read' => 0])->limit(40)->count();
    }
    ...
    {
        return D('Message')->where(['user_id' => session('user.user_id'), 'type' => 1, 'is_read' => 0])->limit(40)->count();
    }
    ...
    {
        return D('Message')->where(['user_id' => session('user.user_id'), 'type' => 2, 'is_read' => 0])->limit(40)->count();
    }
    ...
    public function addSwitchStatusMess ($data)
    {
        $base['num'] = $data['num'];
        $base['title'] = '商品交换提醒';
        $base['type'] = 3;
        ...
        $one['user_id'] = $data['raply_id'];
        $one['user_name'] = $data['raply_name'];
        $one['url'] = '/Switch/showSwitch/switch_id/' . $data['switch_id'];

        $two['user_id'] = $data['user_id'];
        $two['user_name'] = $data['user_name'];
        $two['url'] = '/Switch/showSwitch/switch_id/' . $data['switch_id'];
