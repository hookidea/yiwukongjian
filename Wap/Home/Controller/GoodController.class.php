...
class GoodController extends Controller
{
    ...
    public function getUserList ()
    {
        ...
        if (!$keyword) {
            ...
            $list = $goods->where($where)->field('is_promote,user_id,user_name,good_id,shop_price,promote_price,good_name,thumb_img')->limit($page->limit)->order('add_time desc')->select();
    ...
    public function showGood()
    {
        ...
        $page = new \Org\Util\Page(C('WAP_COMMENT_PAGE_NUM'), false);

        $good_id = I('get.good_id');

        if (IS_AJAX) {

            $commentList = $comment->query('select a.user_id,a.user_name,a.raply_id,a.raply_name,a.content,a.add_time,b.save_path from (select user_id,user_name,raply_id,raply_name,content,add_time from comments where good_id = ' . $good_id . ' order by add_time desc limit ' . $page->limit . ') as a left join images as b on a.user_id = b.user_id');

            $page = $page->show($commentList);
            $commentList = $commentList;
            $this->ajaxReturn(['status' => 1, 'info' => '获取分页成功', 'content' => $commentList, 'page' => $page]);
            ...
            $realRow = $real->where(['user_id' => $row['data']['user_id']])->field('real_location')->find();

            $is_collect = M()->table('collect_goods')->where(['user_id' => session('user.user_id'), 'good_id' => $good_id])->count();


            $imgList = $image->field('good_id,save_path')->where(['good_id'=>$good_id])->select();
            $this->imgList = $imgList;

            $this->is_collect = $is_collect ? 1 : 0;   // 该用户是否已经收藏过该商品
            $this->location = $realRow['real_location'];
            $this->row = $row;

            $commentList = $comment->query('select a.user_id,a.user_name,a.raply_id,a.raply_name,a.content,a.add_time,b.save_path from (select user_id,user_name,raply_id,raply_name,content,add_time from comments where good_id = ' . $good_id . ' order by add_time desc limit ' . $page->limit . ') as a left join images as b on a.user_id = b.user_id');

            $this->page = $page->show($commentList);
